...
- The PRD module(s) matching the changed files. Map in `CLAUDE.md` § "PRD Module Map".
...
- Large functions (>50 lines) or files (>800 lines per `CLAUDE.md`).
...
- `console.log` statements in committed code (`CLAUDE.md` forbids).
...
- Missing SSE reconnection on `visibilitychange` (per §10.4 / `CLAUDE.md`).
...
- Emojis in source (`CLAUDE.md` forbids).
